...
class WalletViewModel: BaseCellViewModel, CollectionSingleSecitionProviderModel, ShouldReactToAction, ShouldPresent {
    ...
    enum ViewToPresent {
        ...
        case cancelDelegatePowerController(CancelDelegatePowerViewModel)
...
fileprivate extension WalletViewModel {
    ...
    func handleItemSelected(_ indexPath: IndexPath) {
        if let item = self.item(at: indexPath) as? WalletMenuCellViewModel {
            ...
            case .cancelDelegate:
                let cancelDelegatePowerViewModel = CancelDelegatePowerViewModel()
                self.setUpTransactionObserers(cancelDelegatePowerViewModel)
                self.shouldPresent(.cancelDelegatePowerController(cancelDelegatePowerViewModel))
            case .cancelPower:
